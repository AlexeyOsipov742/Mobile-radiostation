// wav.c — стриминговое воспроизведение WAV -> MCP4822 (A|B) через spidev.
// Читает WAV блоками (не ест всю RAM), понимает WAVE_FORMAT_EXTENSIBLE.
// Если ValidBits<=12 — выводим «честные 12 бит» без лишней квантизации.
// Сборка: gcc -O2 -Wall -o wav wav.c -lm
// Запуск: ./wav <wav> <spidev> <A|B> <spi_hz> [volume]
...
static inline void ts_from_ns(uint64_t tns, struct timespec*ts){
  ts->tv_sec=(time_t)(tns/1000000000ull); ts->tv_nsec=(long)(tns%1000000000ull);
}
...
int main(int argc, char**argv){
  if (argc < 5){
    fprintf(stderr,"Usage:\n  %s <wav> <spidev> <A|B> <spi_hz> [volume]\n", argv[0]);
    return 1;
  }
  ...
  WavHdr h; if (read(wfd,&h,sizeof(h))!=(ssize_t)sizeof(h)){ fprintf(stderr,"Bad WAV header\n"); close(wfd); return 1; }
  if (h.chunk_id!=0x46464952 || h.format!=0x45564157 || h.sub1_id!=0x20746d66){ fprintf(stderr,"Not RIFF/WAVE 'fmt '\n"); close(wfd); return 1; }
  ...
  if (h.sub1_size>16){
    ...
    if (fmt_tag==0xFFFE && cb>=22){
      ...
      if (lseek(wfd, h.sub1_size-16, SEEK_CUR) < 0){ perror("lseek fmt extra"); close(wfd); return 1; }
    }
  }
  ...
  int sfd=open_spi(spidev,&spi_hz_drv); if(sfd<0){ close(wfd); return 1; }
  ...
  // Кольцевой буфер
  float rb[NTAP]; memset(rb,0,sizeof(rb)); int rpos=0;
  int dec=0; uint32_t rng=0x12345678u;
  ...
  const size_t BYTES_PER_FRAME  = chs * 2; // 16-bit container
  uint8_t *buf = (uint8_t*)malloc(FRAMES_PER_CHUNK * BYTES_PER_FRAME);
  if (!buf){ perror("malloc chunk"); close(sfd); close(wfd); return 1; }

  uint64_t t_next = now_ns();
  size_t out_frames=0;
  int use_raw12 = (valid_bits <= 12); // «честные 12 бит» в 16-б контейнере
  ...
  while (remaining > 0){
    ...
    for (size_t i=0;i<frames;i++){
      ...
      if (++dec == DECIM){
        ...
        float gain = 1.0f;
        if (frame_index < fade_in_len)        gain *= (frame_index / (float)fade_in_len);
        if (remain_out < fade_out_len)        gain *= (remain_out / (float)fade_out_len);
